...
void MyGLWidget::iniEscena ()   // Cal modificar aquesta funció...
{
  ...
  radiEsc = glm::distance(glm::vec3(5,0,5), centreEsc);
  std::cout << "Radi escena: " << radiEsc << std::endl;
  anglegirArbre = 0.0;
  left = right = bottom = top = radiEsc;
}

void MyGLWidget::iniCamera ()   // Cal modificar aquesta funció...
{
  angleTeta = anglePsi = angleFi = 0.0f;
  ra = 1.0f;
  projectTransform ();
  viewTransform ();
}
...
{
  OBS = centreEsc + glm::vec3(cos(angleTeta)*sin(anglePsi), sin(angleTeta), cos(angleTeta)*cos(anglePsi));
  
  glm::mat4 Proj;  // Matriu de projecció
  zn = glm::distance(centreEsc, OBS) - radiEsc - 1; //Li resto 1 perquè no talli les cantonades del terra al rotar
  zf = glm::distance(centreEsc, OBS) + radiEsc;
    
  //ortho l, r, b, t, zNear, zFar
...
{
  //angleTeta -> teta (vertical)
  //anglePsi -> psi (horitzontal)
  //angleFi -> fi
  /*
  //lookAt(OBS,VRP,UP)
  glm::mat4 View = glm::lookAt (OBS, centreEsc, glm::vec3(0,1,0));
  glUniformMatrix4fv (viewLoc, 1, GL_FALSE, &View[0][0]);
  */
  
  glm::mat4 VM (1.0f);
  //VM = glm::rotate(VM, angleFi, glm::vec3(0.0,0.0,1.0));
  VM = glm::rotate(VM, angleTeta, glm::vec3(1.0,0.0,0.0));
  VM = glm::rotate(VM, -anglePsi, glm::vec3(0.0,1.0,0.0));
  VM = glm::translate(VM, -centreEsc);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)  // Cal modificar aquesta funció...
{
  ...
  switch (event->key()) {
    ...
    case Qt::Key_Up: { 
      ...
      if (angleTeta + M_PI/9 <= (M_PI/9) + 0.1) //Li sumo 0.1 per l'error dels decimals de pi 
        angleTeta += M_PI/9;
      viewTransform ();
      break;
    }
