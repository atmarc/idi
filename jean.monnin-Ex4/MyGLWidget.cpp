  ...
  switch (event->key()) {
    case Qt::Key_F: {
      // aquesta tecla ha de canviar cíclicament la posició del focus
        if(typeFocusE == 0) {
            tipusCoordFocus = SCA;
            posFocus = glm::vec3(4.0,3.5,4.0);
            typeFocusE = 1;
        }
        else if (typeFocusE == 1) {
            tipusCoordFocus = SCA;
            posFocus = glm::vec3(1.0,3.5,1.0);
            typeFocusE = 2;
        }
        else if (typeFocusE == 2) {
            tipusCoordFocus = SCO;
            posFocus = glm::vec3(0.0,0.0,0.0);
            typeFocusE = 0;
        }
        emit enviaFocus(typeFocusE);
      break;
    }
...
void MyGLWidget::carregaShaders()
{
  ...
  tipusCoordFocusLoc = glGetUniformLocation (program->programId(), "tipusCoordFocus");
  
  
}

void MyGLWidget::actualitzaFocus(int typeFE) {
      makeCurrent();
      if(typeFE == 0) {
            tipusCoordFocus = SCO;
