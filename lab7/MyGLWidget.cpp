...
void MyGLWidget::viewTransform () {
    ...
    //rotacio2 -> psi
    //rotacio3 -> fi
    rotacio1 = rotacio2 = rotacio3 = 0.0f;
    OBS = nouCentre + glm::vec3(sin(rotacio1), cos(rotacio1)*cos(rotacio2), cos(rotacio1)*sin(rotacio2));
    
    glm::mat4 VM (1.0f);
    VM = glm::translate(VM, glm::vec3 (0.0,0.0,-1.0));//-glm::distance(OBS, nouCentre)));
    VM = glm::rotate(VM, rotacio3, glm::vec3(0.0,0.0,1.0));
    VM = glm::rotate(VM, rotacio1, glm::vec3(1.0,0.0,0.0));
    VM = glm::rotate(VM, rotacio2, glm::vec3(0.0,1.0,0.0));
    VM = VM * glm::translate(VM, glm::vec3 (-nouCentre.x, -nouCentre.y, -nouCentre.z));
    
    glUniformMatrix4fv(viewLoc, 1, GL_FALSE, &VM[0][0]);
    std::cout << -glm::distance(OBS, nouCentre) << std::endl;
    /*
    //lookAt(OBS,VRP,UP)
